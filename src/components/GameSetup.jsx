...
export const GameSetup = ({ onGameStart, playerProfile, onPlayerNameChange, onAvatarChange }) => {
    ...
    const handleStart = () => {
        ...
        onGameStart(gameMode, playerName, p2Name, humanPlayerColor, difficulty, aiMode);
    };
    ...
    return (<div className="bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-md text-white">
            ...
            <div className="space-y-6">
                ...
                {gameMode === 'pva' && (<>
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-2">AI Mode</label>
                            <div className="flex rounded-md shadow-sm">
                                <button onClick={() => setAiMode('offline')} className={`flex-1 py-2 px-4 rounded-l-md transition-colors ${aiMode === 'offline' ? 'bg-indigo-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                                    Offline
                                </button>
                                <button onClick={() => setAiMode('online')} className={`flex-1 py-2 px-4 rounded-r-md transition-colors ${aiMode === 'online' ? 'bg-indigo-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                                    Online (Gemini)
                                </button>
                            </div>
                            {aiMode === 'online' && (<p className="text-xs text-gray-400 mt-2">Online mode requires an internet connection and affects your rank points.</p>)}
                        </div>
                        <div>
